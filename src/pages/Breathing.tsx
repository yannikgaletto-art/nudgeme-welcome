...
const phaseConfig: Record<BreathPhase, { text: string; duration: number; icon?: string }> = {
  inhale: { text: "Inhale", duration: 3, icon: "↑" },
  "hold-top": { text: "Hold", duration: 5 },
  exhale: { text: "Exhale", duration: 7, icon: "↓" },
  "hold-bottom": { text: "Hold", duration: 5 },
};
...
const PATH = {
  A: { x: 40, y: 160 },   // Bottom left (start)
  B: { x: 60, y: 40 },    // Top left
  C: { x: 220, y: 40 },   // Top right
  D: { x: 240, y: 160 },  // Bottom right
};
...
const Breathing = () => {
  ...
  if (showWellDone) {
    return (
      <main
        className={cn(
          "min-h-screen w-full flex items-center justify-center transition-opacity duration-500",
  ...
  return (
    ...
    >
      ...
      <button
        onClick={handleSkip}
        className="absolute top-6 right-5 text-sm font-normal z-10 transition-all duration-200 hover:underline"
      ...
      <div className="flex flex-col items-center">
        ...
        >
          ...
          >
            {phaseConfig[phase].icon && (
              <span className="mr-2">{phaseConfig[phase].icon}</span>
            )}
        ...
        >
          ...
          <path
            d={`M ${PATH.D.x} ${PATH.D.y} L ${PATH.A.x} ${PATH.A.y}`}
            stroke="#FFD4A3"
            strokeWidth="3"
            fill="none"
            strokeLinecap="round"
        ...
        <div className="mt-16">
          <span
            key={phaseTimeLeft}
            className="text-5xl font-semibold animate-timer-pulse"
            style={{ color: "#2C3E50" }}
          >
            {phaseTimeLeft}
          </span>
      ...
      <div className="absolute bottom-8 left-8 right-8 flex flex-col items-center">
        <div
          className="w-full max-w-[400px] h-1.5 rounded-full overflow-hidden"
